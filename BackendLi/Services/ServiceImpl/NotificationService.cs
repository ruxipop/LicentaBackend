...
public class NotificationService : INotificationService
{
    ...
    {
        return _repository.GetEntities<Notification>().Where(l => l.ReceiverId == userId).Include(i => i.Sender)
            .OrderByDescending(n => n.Timestamp).Skip((pageNb - 1) * pageSize).Take(pageSize)
            .ToList();
    }
